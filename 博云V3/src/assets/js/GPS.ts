...
const GPS = {
    ...
    delta: function (lat: number, lon: number): LatLon {
        // Krasovsky 1940
        //
    ...
    deltaOut: function (lat: number, lon: number): LatLon {
        // Krasovsky 1940
        //
        // a = 6378245.0, 1/f = 298.3
        // b = a * (1 - f)
    ...
    //WGS-84 to GCJ-02
    gcj_encrypt: function (wgsLat: number, wgsLon: number): LatLon {
        var d = this.delta(wgsLat, wgsLon);

        return { lat: d.lat, lon: d.lon };
    },
    //GCJ-02 to WGS-84
    gcj_decrypt: function (gcjLat: number, gcjLon: number): LatLon {
    ...
    gcj_decrypt_exact: function (gcjLat: number, gcjLon: number): LatLon {
        var initDelta = 0.01;
    ...
    //GCJ-02 to BD-09
    bd_encrypt: function (gcjLat: number, gcjLon: number): LatLon {
        var x = gcjLon,
            y = gcjLat;
    ...
    //BD-09 to GCJ-02
    bd_decrypt: function (bdLat: number, bdLon: number): LatLon {
        var x = bdLon - 0.0065,
            y = bdLat - 0.006;
        var z = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * this.x_pi);
        var theta = Math.atan2(y, x) - 0.000003 * Math.cos(x * this.x_pi);
    ...
    mercator_encrypt: function (wgsLat: number, wgsLon: number): LatLon {
    ...
    // mercatorLat -> y mercatorLon -> x
    mercator_decrypt: function (mercatorLat: number, mercatorLon: number): LatLon {
    ...
    distance: function (latA: number, lonA: number, latB: number, lonB: number): number {
        var earthR = 6371000;
        var x =
    ...
    defaultToPosition: function (x: any, y: any) { //默认坐标转成position
        //x:43.984348,y:122.386888
        let amapPosition = GPS.delta(x, y)
        //    lat: 43.986719632572296,lon: 122.39272395305956
        return GPS.amapToPosition(amapPosition.lon, amapPosition.lat)
    },
    defaultToPositionList: function ({ list, xKey, yKey }: {list: any, xKey: any, yKey: any}) { //批量默认坐标转成position
        let result: any = {};
        let tepList: any = []
        list.map((item: { [x: string]: any; }) => {
            tepList.push(GPS.defaultToPosition(item[xKey], item[yKey]));
        });
        tepList.map((item: any) => {
    ...
    amapToPosition: function (x: number, y: number) { //高德地图坐标到position
    ...
    amapToPositionList: function ({ list, xKey, yKey }: {list: any, xKey: any, yKey: any}) { //批量高德坐标转成批量position
        /*
        list=[
    ...
    positioListTran: function (list: any[]) {
        /*
            [
                {
                    amap:{1a},
         ...
         */
        let result: any = {}
        list.map((item: { [x: string]: any; }, index: string | number) => {
