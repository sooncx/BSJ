...
instance.interceptors.request.use(
  (oldConfig: AxiosRequestConfig) => {
    const newConfig = {
      ...oldConfig,
      url: (oldConfig.url?.slice(0, 4) === "http" || oldConfig.url?.slice(0, 5) === "https") ? oldConfig.url : (process.env.NODE_ENV === "development" ? baseURL : CONFIG.baseUrl) + oldConfig.url,
    ...
    newConfig.headers.sessionId = store.state.sessionId; // 测试用，注意删掉
    return newConfig;
  },
