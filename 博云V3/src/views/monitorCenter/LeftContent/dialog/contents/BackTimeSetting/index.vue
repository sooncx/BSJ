<template>
  <div class="title">
    <p>请输入回传间隔(单位：秒 范围：3~10800)</p>
    <span></span>
  </div>
  <div class="BackTimeSetting">
    <el-form
      ref="ruleForm"
      :rules="rules"
      :model="formData"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const checkSecond = (rule: any, value: any, callback: any) => {
      if (!value) {
        return callback(new Error("回传间隔值不能为空"));
      }
      setTimeout(() => {
        if (value < 3 || value > 10800) {
          callback(new Error("范围3~10800"));
        } else {
          callback();
        }
      }, 100);
    ...
    const data = reactive({
      ...
      async sendCommand() {
        ...
        let params = {
          ...
          commandMsg: data.formData.second,
        } as any;
      ...
      rules: {
        second: [{ validator: checkSecond, trigger: "blur" }],
      },
    });
    watch(
      () => props.visible,
