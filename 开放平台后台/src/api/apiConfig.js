...
if (process.env.NODE_ENV === "development") {
  ...
  instance.defaults.baseURL = baseURL; // 生产环境
}
...
instance.interceptors.request.use(
  oldConfig => {
    const newConfig = {
      ...
      sessionId: store.state.USER ? store.state.USER.token : "",
      adapter: oldConfig.adapter === "jsonp" ? axiosJsonp : oldConfig.adapter
    };
    newConfig.headers.sessionId = store.state.USER ? store.state.USER.token : "";
    return newConfig;
  },
